...
export interface ProjectAttendanceMapItem {
  name: string;
  employeeId?: string;
  status: string | null;
  address: string;
...
interface ProjectAttendanceMapProps {
...
export function ProjectAttendanceMap({ items }: ProjectAttendanceMapProps) {
  ...
  // More precise bounds for Indonesia archipelago
  const indonesiaBounds = L.latLngBounds(
    L.latLng(-11.0, 94.0),  // southwest (includes westernmost islands)
    L.latLng(6.5, 141.0)    // northeast (includes Papua, excludes far eastern islands)
  );
  ...
  useEffect(() => {
    ...
    const map = L.map(containerRef.current, {
      ...
      maxBoundsViscosity: 1.0, // Strict enforcement - prevents panning outside bounds
      worldCopyJump: false, // Prevent wrapping around the world
      minZoom: 4, // Prevent zooming out too far (was 1, now 4 to keep Indonesia in view)
      maxZoom: 18,
    });

    // Strictly enforce Indonesia bounds
    map.setMaxBounds(indonesiaBounds);
    
    // Add event listener to ensure map stays within bounds on every move
    map.on('drag', () => {
      const bounds = map.getBounds();
      if (!indonesiaBounds.contains(bounds)) {
        // If map is dragged outside bounds, reset to valid position
    ...
    // Prevent zooming out beyond Indonesia bounds
    map.on('zoomend', () => {
      const bounds = map.getBounds();
      if (!indonesiaBounds.contains(bounds)) {
        // If zoomed out too far, fit to Indonesia bounds
        map.fitBounds(indonesiaBounds, { padding: [40, 40], maxZoom: map.getZoom() });
      }
    });
    
    const tileLayer = L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
      attribution: '&copy; OpenStreetMap contributors',
      bounds: indonesiaBounds, // Limit tile loading to Indonesia bounds
    });
    tileLayer.addTo(map);
  ...
  useEffect(() => {
    ...
    const grouped = new Map<
      string,
      { lat: number; lng: number; items: ProjectAttendanceMapItem[] }
    >();
    ...
    grouped.forEach(({ lat, lng, items }) => {
      ...
      items.forEach((p, index) => {
        ...
        marker.bindPopup(
          ...
          ${p.role ? `<div style="color:#6b7280;font-size:10px;margin-bottom:3px;">Role: ${p.role}</div>` : ""}
          ${p.department ? `<div style="color:#6b7280;font-size:10px;margin-bottom:3px;">Department: ${p.department}</div>` : ""}
    ...
    if (bounds.length > 0) {
      ...
      // Constrain marker bounds to Indonesia bounds
      const southWest = boundsLatLng.getSouthWest();
      const northEast = boundsLatLng.getNorthEast();
      
      const constrainedSouth = Math.max(indonesiaBounds.getSouth(), southWest.lat);
      const constrainedWest = Math.max(indonesiaBounds.getWest(), southWest.lng);
      const constrainedNorth = Math.min(indonesiaBounds.getNorth(), northEast.lat);
      const constrainedEast = Math.min(indonesiaBounds.getEast(), northEast.lng);
      ...
      // Ensure constrained bounds are valid
      if (constrainedBounds.isValid()) {
        ...
        // Fallback: fit to Indonesia bounds if marker bounds are invalid
      ...
      // Strictly enforce Indonesia bounds after fitting
      ...
      <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <span className={`text-xs ${darkMode ? "text-gray-300" : "text-gray-500"}`}>
            {safeItems.length} active {safeItems.length === 1 ? "member" : "members"}
          </span>
          <button
            type="button"
