  ...
  const onSubmit = async (value: CandidateSchemaType) => {
    startLoading(async () => {
      ...
      const data: PutCandidateDto = {
        ...
        total_experience: value.total_experience ?? 0,
        highest_degree: value.highest_degree || "",
        salary_expectation: value.salary_expectation ?? 0,
        qualified_criteria1: value.qualified_criteria1 ?? false,
        qualified_criteria2: value.qualified_criteria2 ?? false,
        date_standardized: value.date_standardized?.toISOString() || "",
        form_timestamp: value.form_timestamp?.toISOString() || "",
        work_experience: value.work_experience || "",
        last_project_description: value.last_project_description || "",
        primary_programming_language: value.primary_programming_language || "",
        programming_language_experience: value.programming_language_experience || "",
        operating_systems: value.operating_systems || "",
        frameworks_libraries: value.frameworks_libraries || "",
        server_experience: value.server_experience || "",
        domicile: value.domicile || "",
        processed_status: detail.processedStatus,
