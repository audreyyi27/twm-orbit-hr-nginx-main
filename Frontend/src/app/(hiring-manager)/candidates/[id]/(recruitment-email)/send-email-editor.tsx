...
export default function SendEmailEditor(props: { defaultLoading?: boolean }) {
  ...
  useEffect(() => {
    ...
    startLoading(async () => {
      if (!detail?.id) return
      const res = await GetEmailTemplateService(detail.id, emailTemplateType as unknown as Record<string, unknown>)
  ...
  const handleSendEmail = () => {
    ...
    startLoading(async () => {
      ...
      const res = await PostSendEmailService(detail?.id, data as unknown as Record<string, unknown>)
      if (res.isError) {
