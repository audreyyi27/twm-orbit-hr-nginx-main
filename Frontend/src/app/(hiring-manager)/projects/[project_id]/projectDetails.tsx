...
import type { ProjectDetailsDto } from '@/core/projects';
...
export default function ProjectDetailsClient({ projectId, data }: ProjectDetailsClientProps) {
  const router = useRouter();
  const [isAddMemberModalOpen, setIsAddMemberModalOpen] = useState(false);
  const [isDeleteMode, setIsDeleteMode] = useState(false);
  const [deletingMemberId, setDeletingMemberId] = useState<string | null>(null);
  const { project, team, members } = data;
  ...
  const statusStyle = statusConfig[project.status?.toLowerCase() as keyof typeof statusConfig] || statusConfig.active;
  ...
  const handleRemoveMember = async (taskId: string, memberName: string) => {
    ...
    try {
      ...
      if (result.isError) {
        ...
        console.error('Remove member error:', result);
        return;
      }

      // Exit delete mode and refresh the page data
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600">
              {members.length} {members.length === 1 ? 'member' : 'members'} working on this project
            </p>
          </div>
        ...
        {members.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {members.map((member) => (
              <div
                key={member.task_id}
                className="bg-gradient-to-br from-gray-50 to-white rounded-lg border-2 border-gray-200 p-5 hover:border-blue-300 transition-all relative"
              >
                {/* Delete Button - Only visible in delete mode */}
                {isDeleteMode && (
                  <button
                    onClick={() => handleRemoveMember(member.task_id, member.name || 'this member')}
                    disabled={deletingMemberId === member.task_id}
                  ...
                  >
                    {deletingMemberId === member.task_id ? (
