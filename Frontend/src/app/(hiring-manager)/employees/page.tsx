...
export default async function EmployeesPage({ searchParams }: PageProps) {
  ...
  const totalPages = (meta as Record<string, unknown> | undefined)?.total_pages as number || 1; 
  ...
  return (
    <div className="p-6">
      ...
      {/* Pagination */}
      {meta && typeof meta === 'object' && 'total_pages' in meta && (meta as any).total_pages > 0 && (
        <div className="flex items-center justify-between mt-4">
          <div className="text-sm text-gray-600">
            Page {(meta as any).page} of {(meta as any).total_pages}
          </div>
          
          <div className="flex gap-2">
            {/* Previous */}
            {(meta as any).page > 1 ? (
              <Link href={`/employees?page=${(meta as any).page - 1}&per_page=${(meta as any).per_page}${search ? `&search=${search}` : ''}`}>
            ...
            {/* Next */}
            {(meta as any).page < (meta as any).total_pages ? (
              <Link href={`/employees?page=${(meta as any).page + 1}&per_page=${(meta as any).per_page}${search ? `&search=${search}` : ''}`}>
