...
export interface CandidateDto {
  ...
  // Date and position
  date_standardized?: string;  // ISO timestamp
  form_timestamp?: string;  // ISO timestamp
  applied_as?: string;  // junior developer / senior developer
  
  // Status
  candidate_status?: string;  // CandidateStatusEnum from backend
  qualified_criteria1?: boolean;
  qualified_criteria2?: boolean;
  
  // CV
  cv_file?: string;
  work_experience?: string;
  last_project_description?: string;
  primary_programming_language?: string;
  programming_language_experience?: string;
