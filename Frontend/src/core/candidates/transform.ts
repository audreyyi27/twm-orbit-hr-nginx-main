...
export function TransformDetailCandidate(data: CandidateDto): Candidate {
  ...
  return {
    ...
    attendStatus: "",
    dateStandardized: data.date_standardized || "",
    finalDecision: "",
    ...
    // Add raw database fields for easy access in detail page
    rawData: {
      uuid: candidateId,
      name: data.name || "",
      email: data.email || "",
      ...
      pengalaman_total: data.total_experience || data.experience_month || 0,
      gelar_tertinggi: data.highest_degree || "",
      minat: data.job_preference || "",
      preferensi_lokasi: data.location_preference || "",
      ...
      pengalaman_organisasi_relawan: data.volunteer_organization_experience || "",
      penghargaan_dari_preferensi: data.awards_from_preference || "",
      ekspektasi_gaji: data.salary_expectation?.toString() || "",
      tentang_saya: data.about_me || "",
      skills: data.skills || "",
      pendidikan: data.education || "",
      penghargaan: data.awards || "",
      organisasi: data.organization || "",
      cv_file: data.cv_file || "",
      date_scraped: data.date_standardized || "",
      applied_as: data.applied_as || "",  // junior/senior developer
    }
  };
}
