...
export interface Candidate {
  ...
  appliedAt?: string;
  // Raw database fields - all 29 columns from your DB
  rawData?: {
    uuid: string;
    name: string;
    email: string;
    ...
    umur: number;  // age
    gender: string;
    lokasi: string;  // domicile
    pengalaman_total: number;  // total experience
    gelar_tertinggi: string;  // highest degree
    minat: string;  // interests
    preferensi_lokasi: string;  // preferred location
    instagram: string;
    linkedin: string;
    github: string;
    codepen: string;
    facebook: string;
    twitter: string;
    sertifikat: string;  // certificates
    pengalaman_organisasi_relawan: string;  // volunteer/org experience
    penghargaan_dari_preferensi: string;  // awards from preference
    ekspektasi_gaji: string;  // salary expectation
    tentang_saya: string;  // about me
    skills: string;
    pendidikan: string;  // education
    penghargaan: string;  // awards
    organisasi: string;  // organizations
    cv_file: string;
    date_scraped: string;
    applied_as: string;  // junior/senior developer
  };
}
